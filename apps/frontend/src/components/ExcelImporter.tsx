...
export const ExcelImporter: React.FC<{ onImportComplete?: () => void }> = ({ onImportComplete }) => {
        ...
        {result && (
          <div className="space-y-2">
            <div className={`flex items-center gap-2 p-3 rounded-md ${
              result.imported > 0 ? 'bg-green-50 text-green-900' : 'bg-red-50 text-red-900'
            }`}>
              {result.imported > 0 ? (
              ...
              <div className="text-sm">
                <strong>{result.imported}</strong> posts imported successfully
                {result.errors > 0 && (
